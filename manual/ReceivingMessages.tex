...
An event-based actor, i.e., the default implementation in \ba, uses \lstinline^become^ to set its behavior.
...
\textbf{Note}: the message handling in \ba is consistent among all actor implementations: unmatched messages are \textit{never} implicitly discarded if no suitable handler was found.
...
\ba supports timeouts using \lstinline^minutes^, \lstinline^seconds^, \lstinline^milliseconds^ and \lstinline^microseconds^.
...
Unmatched messages are skipped automatically by \ba's runtime system.
