...
abstract_actor_ptr remote_actor_impl(stream_ptr_pair io, string_set expected) {
    ...
    if (*pinf == *pinfptr) {
        ...
        auto ptr = singletons::get_actor_registry()->get(remote_aid);
        return ptr;
    }
    struct remote_actor_result {
        remote_actor_result* next;
        boost::actor::actor value;
    };
    std::mutex qmtx;
    std::condition_variable qcv;
    boost::actor::detail::single_reader_queue<remote_actor_result> q;
