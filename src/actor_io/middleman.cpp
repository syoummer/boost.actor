...
class middleman_event {
    ...
    friend class boost::actor::detail::single_reader_queue<middleman_event>;

 public:

    template<typename Arg>
    middleman_event(Arg&& arg) : next(nullptr), fun(std::forward<Arg>(arg)) { }
...
typedef boost::actor::detail::single_reader_queue<middleman_event> middleman_queue;
...
class middleman_impl : public middleman {
 ...
 public:
    ...
    bool register_peer(const node_id& node, peer* ptr) override {
        ...
        if (entry.impl == nullptr) {
            if (entry.queue == nullptr) {
                entry.queue = make_counted<default_message_queue>();
            }
    ...
    void register_acceptor(const actor_addr& aa, peer_acceptor* ptr) override {
        run_later([=] {
            BOOST_ACTOR_LOGC_TRACE("cppa::middleman",
...
#       endif
        ...
        m_namespace.set_proxy_factory([=](actor_id aid, node_id_ptr ptr) -> actor_proxy_ptr {
            auto res = make_counted<remote_actor_proxy>(aid, std::move(ptr), this);
