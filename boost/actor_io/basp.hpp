...
#ifndef BOOST_ACTOR_IO_BASP_HPP
#define BOOST_ACTOR_IO_BASP_HPP
 ...
 */
constexpr size_t header_size =   actor::node_id::host_id_size * 2
                               + sizeof(uint32_t) * 2
                               + sizeof(actor::actor_id) * 2
                               + sizeof(uint32_t) * 2
                               + sizeof(uint64_t);
...
inline bool valid(header& hdr) {
    switch (hdr.operation) {
        default:                      return false; // invalid operation field
        case server_handshake:        return server_handshake_valid(hdr);
        case client_handshake:        return client_handshake_valid(hdr);
        case dispatch_message:        return dispatch_message_valid(hdr);
        case announce_proxy_instance: return announce_proxy_instance_valid(hdr);
        case kill_proxy_instance:     return kill_proxy_instance_valid(hdr);
    }
}
