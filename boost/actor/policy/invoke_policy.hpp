...
#ifndef BOOST_ACTOR_INVOKE_POLICY_HPP
#define BOOST_ACTOR_INVOKE_POLICY_HPP
        ...
        if (res) {
            ...
            if (res->empty()) {
                ...
                if (mid.is_request() && !mid.is_answered()) {
                    BOOST_ACTOR_LOG_WARNING("actor with ID " << self->id()
                                     << " did not reply to a "
                ...
                if (   detail::matches<atom_value, std::uint64_t>(*res)
                    ...
                    BOOST_ACTOR_LOG_DEBUG("respond via response_promise");
                    auto fhdl = fetch_response_promise(self, hdl);
                    if (fhdl) {
    ...
    handle_message_result handle_message(Actor* self,
        ...
        switch (this->filter_msg(self, node)) {
            case msg_type::normal_exit: {
                BOOST_ACTOR_LOG_DEBUG("dropped normal exit signal");
                return hm_drop_msg;
            }
            case msg_type::expired_sync_response: {
                BOOST_ACTOR_LOG_DEBUG("dropped expired sync response");
                return hm_drop_msg;
            }
            case msg_type::expired_timeout: {
                BOOST_ACTOR_LOG_DEBUG("dropped expired timeout message");
                return hm_drop_msg;
            }
            case msg_type::inactive_timeout: {
                BOOST_ACTOR_LOG_DEBUG("skipped inactive timeout message");
                return hm_skip_msg;
            }
            case msg_type::non_normal_exit: {
                BOOST_ACTOR_LOG_DEBUG("handled non-normal exit signal");
                // this message was handled
                // by calling self->quit(...)
                return hm_msg_handled;
            }
            case msg_type::timeout: {
                BOOST_ACTOR_LOG_DEBUG("handle timeout message");
            ...
            case msg_type::sync_response: {
                BOOST_ACTOR_LOG_DEBUG("handle as synchronous response: "
                               << BOOST_ACTOR_TARG(node->msg, to_string) << ", "
                               << BOOST_ACTOR_MARG(node->mid, integer_value) << ", "
                               << BOOST_ACTOR_MARG(awaited_response, integer_value));
                if (awaited_response.valid() && node->mid == awaited_response) {
                    ...
                    if (!res && handle_sync_failure_on_mismatch) {
                        BOOST_ACTOR_LOG_WARNING("sync failure occured in actor "
                                         << "with ID " << self->id());
            ...
            case msg_type::ordinary: {
                ...
                BOOST_ACTOR_LOG_DEBUG_IF(awaited_response.valid(),
        ...
        BOOST_ACTOR_CRITICAL("invalid message type");
    }
...
#endif // BOOST_ACTOR_INVOKE_POLICY_HPP
