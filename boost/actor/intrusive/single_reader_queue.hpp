...
#ifndef BOOST_ACTOR_SINGLE_READER_QUEUE_HPP
#define BOOST_ACTOR_SINGLE_READER_QUEUE_HPP
...
class single_reader_queue {
 ...
 public:
    ...
    inline bool can_fetch_more() {
        ...
        BOOST_ACTOR_REQUIRE(ptr != nullptr);
        return !is_dummy(ptr);
    }
     ...
     */
    inline bool empty() {
        BOOST_ACTOR_REQUIRE(m_stack.load() != nullptr);
    ...
    bool synchronized_enqueue(Mutex& mtx, CondVar& cv, pointer new_element) {
        ...
        BOOST_ACTOR_CRITICAL("invalid result of enqueue()");
    }
 ...
 private:
    ...
    bool fetch_new_data(pointer end_ptr) {
        BOOST_ACTOR_REQUIRE(m_head == nullptr);
        BOOST_ACTOR_REQUIRE(!end_ptr || end_ptr == stack_empty_dummy());
        pointer e = m_stack.load();
        // must not be called on a closed queue
        BOOST_ACTOR_REQUIRE(e != nullptr);
        ...
        while (e != end_ptr) {
            if (m_stack.compare_exchange_weak(e, end_ptr)) {
                if (is_dummy(e)) {
                    ...
                    BOOST_ACTOR_REQUIRE(end_ptr == nullptr);
                    return false;
                }
                while (e) {
                    BOOST_ACTOR_REQUIRE(!is_dummy(e));
                    auto next = e->next;
                    e->next = m_head;
                    m_head = e;
                    e = next;
