...
#ifndef BOOST_ACTOR_FIXED_VECTOR_HPP
#define BOOST_ACTOR_FIXED_VECTOR_HPP
...
class limited_vector {
 ...
 public:
    ...
    void resize(size_type s) {
        BOOST_ACTOR_REQUIRE(s <= MaxSize);
        m_size = s;
    }

    limited_vector(std::initializer_list<T> init) : m_size(init.size()) {
        BOOST_ACTOR_REQUIRE(init.size() <= MaxSize);
    ...
    inline reference at(size_type pos) {
        BOOST_ACTOR_REQUIRE(pos < m_size);
        return m_data[pos];
    }
    ...
    inline void insert(iterator pos, InputIterator first, InputIterator last) {
        BOOST_ACTOR_REQUIRE(first <= last);
...
#endif // BOOST_ACTOR_FIXED_VECTOR_HPP
