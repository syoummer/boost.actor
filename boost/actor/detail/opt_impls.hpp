...
#ifndef BOOST_ACTOR_OPT_IMPLS_HPP
#define BOOST_ACTOR_OPT_IMPLS_HPP
...
typedef decltype(on<std::string>().when(placeholders::_x1.in(std::vector<std::string>())))
        opt0_rvalue_builder;
...
class opt1_rvalue_builder {
 ...
 public:
    ...
    typedef decltype(on<std::string, std::string>()
                    .when(placeholders::_x1.in(std::vector<std::string>())))
            left_type;
...
#endif // BOOST_ACTOR_OPT_IMPLS_HPP
